		...
		try {
			...
			do {
				...
				case 3:
					...
					String c1 = s.next();
					shop.stream().filter(p -> p.getPtype().equals(c1)).forEach(a -> System.out.println(a));

					break;

				case 4:
					shop.stream().map(x -> x.getPname()).forEach(q -> System.out.println(q));
					break;

				case 5:
					shop.stream().filter(x -> x.getPrice() > 200).forEach(q -> System.out.println(q));
					break;

				case 6:
					double totalprice = shop.stream().mapToDouble(p -> p.getPrice()).sum();
